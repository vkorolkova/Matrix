...
public:
	...
	Fraction Cancellation(Fraction &a)
	{
		if (a.numerator != 0)
		{
			...
			int n = a.numerator,
				m = a.denominator;
			// вычисление Ќќƒ(числитель, знаменатель)
			// алгоритмом ≈вклида
			while (n != m) {
				if (n > m) {
					n -= m;
	...
	friend ostream& operator << (ostream& out, Fraction& a) {
		// знаменатель печатаетс€, если числитель не равен 0
		if (a.numerator == 0) {
			out << "0";
		}
		else {
	...
	// операци€ ввода рациональной дроби
	...
	void Gauss() {
		...
		while (k < n)
		{
			...
			for (int i = k; i < n; i++) {
				...
				if (mod.mod(tem) < eps) continue; // дл€ нулевого коэффициента пропустить
				for (int j = 0; j < n; j++)
